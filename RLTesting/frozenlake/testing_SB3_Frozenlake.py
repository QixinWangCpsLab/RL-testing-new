...
from Frozenlake_Env import EnvWrapper
...
def round_loop(config, rounds=25, epochs=300, bug_list=[], model_type='dqn'):
    ...
    for round in range(rounds):
        ...
        log_dir = os.path.join(config['root_dir'], 'RLTesting', 'logs', model_type, str(bug_list))
...
# 默认使用dqn进行训练
def main(bug_version_list, model_type='dqn'):
    ...
    for bug_version in bug_version_list:
        ...
        round_loop(config=config, rounds=25, epochs=300, bug_list=bug_version, model_type=model_type)
